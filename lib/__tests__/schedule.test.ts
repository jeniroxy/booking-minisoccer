...
describe('getSlotStatus', () => {
  ...
  it('returns pending for a pending booking', () => {
    const bookings: Booking[] = [
      { id: 'bk1', team_name: 'A', booking_date: '2026-03-28', time_slot_id: 'slot-1', status: 'pending', total_price: null, phone: null, created_at: '' },
    ]
  ...
  it('returns confirmed for a confirmed booking', () => {
    const bookings: Booking[] = [
      { id: 'bk1', team_name: 'A', booking_date: '2026-03-28', time_slot_id: 'slot-1', status: 'confirmed', total_price: null, phone: null, created_at: '' },
    ]
  ...
  it('returns available when booking is cancelled (slot freed)', () => {
    const bookings: Booking[] = [
      { id: 'bk1', team_name: 'A', booking_date: '2026-03-28', time_slot_id: 'slot-1', status: 'cancelled', total_price: null, phone: null, created_at: '' },
    ]
    const { status } = getSlotStatus(slot, '2026-03-28', bookings, [], '2026-03-27')
    expect(status).toBe('available')
  })
